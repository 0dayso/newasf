...
class RoleAction extends CommonAction {
    ...
    public function user()
    {
     ...
     //   $where['f']=1;
		$list2=$user->field('id,username,name,company_id,department_id,position_id,status')->where($map)->select();
		//echo $user->getlastsql();
		//print_r($list2);
		foreach ($list2 as $vo){
			$userList[$vo['id']]	=	$vo['username'].' - '.$vo['company_id'].' - '.$vo['department_id'].' -  '.$vo['position_id'].' -  '.$vo['status'];
		}
