...
function member_find($shyzcm,$shykh='',$ssj='',$sjc=''){
     ...
     */
    function insert_db(){
        $rs=$this->member_find('liudongyun');
        foreach($rs as $key=>$val){
            $rs[$key]['info']=$this->get_member_info($val[8]);
        }
        print_r($rs);
    ...
    function repeatUsername($name){
        if($this->member_find($name)){
            $this->setError("会员名已存在");//会员注册名
        }elseif( $this->member_find('',$name)){
            $this->setError("会员号已存在");//会员会员卡号
        }elseif($this->member_find('','',$name)){
            $this->setError("手机号已存在");//手机号已存在
        }else{
            return true;
        }
    }
...
//会员注册
    function registration($in1,$in2,$in3){
        ...
        $post_arr['ywyid']='6000';
        $post_arr['by01']='';
        $rs=curl_post('http://121.8.201.163:8150/asms/member/t_member_save.shtml',$post_arr,COOKIE_FILE);
        return ($rs);
        $str=<<<xml
<?xml version="1.0" encoding="UTF-8"?>
<response>
